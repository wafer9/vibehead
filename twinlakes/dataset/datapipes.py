...
from twinlakes.dataset.processor import parse_url
...
@functional_datapipe("flat_map_ignore_error")
class FlatMapperIgnoreErrorDataPipe(Mapper):
    """ Like map_ignore_error, but `fn` returns an iterable of samples; each is
        yielded individually (one-to-many). Errors from a single source element
        are logged and skipped without aborting the whole stream. """
    ...
    def __iter__(self):
        ...
        while True:
            try:
                elem = next(self._iter)
                for new_elem in self._apply_fn(elem):
                    yield new_elem
            except StopIteration:
                self._iter = None
                return
...
class TarsDataPipe(IterDataPipe):
    ...
    def __iter__(self):
        from twinlakes.dataset.processor import AUDIO_FORMAT_SETS
                ...
                with tarfile.open(fileobj=sample['stream'],
                    ...
                    for tarinfo in stream:
                        ...
                        with stream.extractfile(tarinfo) as file_obj:
                            try:
                                if postfix == 'text':
                                    example['text'] = file_obj.read().decode(
                                        'utf8').strip()
                                elif postfix == 'label':
                                    example['label'] = file_obj.read().decode(
                                        'utf8').strip()
